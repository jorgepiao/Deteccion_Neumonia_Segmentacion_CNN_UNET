...
# Cargar el modelos de la Red Neuronal Convolucional y segmentacion UNET
rnc_model = load_model('modelo_rnc/rnc_model_segm.h5')
rnc_model_covid = load_model('modelo_rnc/rnc_model_covid.h5')
with CustomObjectScope({'iou': iou, 'dice_coef': dice_coef, 'dice_loss': dice_loss}):
    model_segm = load_model('modelo_rnc/model_segm.h5')
...
class RadiografiaMuestra(VentanaImagen):
    ...
    def ventana_radiografia_muestra(self, tabla, ruta_guardar_img):
        ...
        fila = rand.choice(list(filas_lista.keys()))
...
class RedConvolucional(VentanaImagen):
    ...
    def prediccion_neumonia(self, radiografia=None):
        ...
        imagen = cv2.imread(radiografia, cv2.IMREAD_COLOR)
        imagen_x = cv2.resize(imagen, (512, 512))
        x = imagen_x/255.0
        ...
        mm = MatteMatting(radiografia, save_image_path)
        ...
        np_image = cv2.imread(f"{save_image_path}.png")
        # Reescalar
        np_image = skimage.transform.resize(np_image, (150,150,3), mode='constant', anti_aliasing=True)
        # Imagen en gris (blanco y negro)
        self.np_imagen = rgb2gray(np_image)
        # Convertir la mascara a array de numpy y agregarla a una lista
        self.np_imagen = np.asarray(self.np_imagen)
